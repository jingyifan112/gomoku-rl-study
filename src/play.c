...
static int read_human_move(GameState *state) {
    int move;
    ...
    while (1) {
        ...
        if (move < 0 || move >= BOARD_CELLS) {
            printf("Move out of range. Please enter 0-%d.\n", BOARD_CELLS - 1);
            continue;
        }
...
static void play_game(NeuralNetwork *nn) {
    ...
    printf("Welcome to 15x15 Gomoku!\n");
    printf("You are X. The computer is O.\n");
    printf("Enter a board position number to make a move.\n");
...
int main(int argc, char **argv) {
    ...
    if (!load_neural_network(&nn, model_file)) {
        printf("Could not load model. Please run training first:\n");
        printf("  ./train 1000\n");
        return 1;
    }
